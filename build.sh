...
function setup() {
    ...
    exit_code=$(
        sfdx force:org:create -f config/project-scratch-def.json --setdefaultusername --durationdays 1 --wait 3 2>"$TMPDIR/err"
        echo $?
    )
...
function ci_test_status() {
    outcome=$(jq '.result.summary.outcome' <"$TMPDIR/test_result.json")
    if [ "$outcome" != '"Passed"' ]; then
        pp "${red}At least one test failed."
        problems=$((problems + 1))
    fi
}

function ci_coverage_status() {
    outcome=$(jq '.result.summary.testRunCoverage' <"$TMPDIR/test_result.json" | tr -d "%\"")
    if [ "$outcome" -lt "$CODE_COVERAGE_THRESHOLD" ]; then
        pp "${red}Insufficient code coverage."
        problems=$((problems + 1))
    fi
}
...
# Minimum code coverage value in percent to pass
