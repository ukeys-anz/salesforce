...
const findSurpressedPMDRules = (
  ...
  for (let ind = 0; ind < classLines.length; ind++) {
    ...
    if (line.indexOf(specifidTestSymbol) > -1) {
      const surpressedLine = line
        .split(specifidTestSymbol)[1]
        .replace("('", "")
        .replace("')", "")
        .replaceAll(" ", "");
      const surpressedOnNextLine = classLines[ind + 1]
        .replaceAll(" ", "")
        .replaceAll("'", "");
      const pmdRuleSurpressed =
        surpressedLine === "(" ? surpressedOnNextLine : surpressedLine;
