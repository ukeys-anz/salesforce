...
var _CompletedQARelatedList = require("./../pageObjects/completedQARelatedList");
...
var _CompletedQARelatedList__default = /*#__PURE__*/ _interopDefaultLegacy(
  _CompletedQARelatedList
);
...
async function _utam_get_completedQARelatedList(driver, root) {
  ...
  const _locator = core.By.css(`.colSide > .flexipageComponent:nth-of-type(2)`);
  return _element.findElement(_locator);
}

async function _utam_get_listViewByTitles(driver, root) {
  let _element = await _utam_get_homePage(driver, root);
  const _locator = core.By.css(`.flexipageFilterListCard`);
  return _element.findElements(_locator);
}

class CoachesWorkbenchHomePage extends core.UtamBaseRootPageObject {
  ...
  async getCompletedQARelatedList() {
    ...
    element = new _CompletedQARelatedList__default["default"](driver, element);
...
module.exports = CoachesWorkbenchHomePage;
