...
async function _utam_get_caseRecordTypes(driver, root) {
  let _element = root;
  const _locator = _By.css(`article[class*='cNewCaseAuraWrapper'] span`);
  const hasElement = await _element.containsElement(_locator);
  if (!hasElement) {
    return null;
  }
...
async function _utam_get_fieldByLabels(driver, root) {
  let _element = root;
  const _locator = _By.css(`.forcePageBlockItem`);
  const hasElement = await _element.containsElement(_locator);
  if (!hasElement) {
    return null;
  }
...
async function _utam_get_picklistss(driver, root) {
  let _element = root;
  const _locator = _By.css(`.desktop > .select-options ul`);
  return _element.findElements(_locator);
}
...
export default class RecordCreationForm extends _UtamBaseRootPageObject {
  ...
  async __getFormSectionRowItem(
    ...
    element = new _RecordCreationFormField(driver, element);
  ...
  async getFieldByLabel(label) {
    ...
    elements = elements.map(function _createElement(element) {
      return new _RecordCreationFormField(driver, element);
    });
    const appliedFilter = await Promise.all(
  ...
  async getPicklists() {
    ...
    elements = elements.map(function _createElement(element) {
      return new _RecordCreationFormPicklist(driver, element);
    });
    await Promise.all(elements.map((el) => el.__beforeLoad__()));
    return elements;
  }
