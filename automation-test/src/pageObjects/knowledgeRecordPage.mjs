...
import _ArticleCategoriesEditorModal from "./../pageObjects/articleCategoriesEditorModal";
...
async function _utam_get_main(driver, root) {
  let _element = root;
  const _locator = _By.css(`.row-main`);
  return _element.findElement(_locator);
}
...
export default class KnowledgeRecordPage extends _UtamBasePageObject {
  ...
  async getHistories() {
    ...
    elements = elements.map(function _createElement(element) {
      return new _KnowledgeApprovalHistory(driver, element);
    });
  ...
  async __getArticleCategoriesEditorModal() {
    ...
    element = new _ArticleCategoriesEditorModal(driver, element);
