...
async function _utam_get_tabBarItemss(driver, root) {
  let _element = root;
  const _locator = _By.css(`.tabsetHeader ul.tabBarItems > li.tabItem`);
...
async function _utam_get_currentTab(driver, root) {
  let _element = root;
  const _locator = _By.css(`div[class*='selectedListItem'] a`);
  return _element.findElement(_locator);
}
...
export default class ConsoleAppNavigation extends _UtamBaseRootPageObject {
  ...
  async getTabBarItems() {
    ...
    elements = elements.map(function _createElement(element) {
      return new _ConsoleTabBarItem(driver, element);
    });
