...
var _AlohaPage = require("./../pageObjects/alohaPage");

function _interopDefaultLegacy(e) {
  return e && typeof e === "object" && "default" in e ? e : { default: e };
}

var _AlohaPage__default = /*#__PURE__*/ _interopDefaultLegacy(_AlohaPage);

async function _utam_get_label(driver, root) {
  let _element = root;
  const _locator = core.By.css(`.label span:nth-child(1)`);
...
async function _utam_get_readOnlylabel(driver, root) {
  let _element = root;
  const _locator = core.By.css(`.slds-form-element__label`);
  const hasElement = await _element.containsElement(_locator);
  if (!hasElement) {
    return null;
  }
...
async function _utam_get_itemBody(driver, root) {
  let _element = root;
  const _locator = core.By.css(`.itemBody`);
  const hasElement = await _element.containsElement(_locator);
  if (!hasElement) {
    return null;
  }
...
async function _utam_get_editButton(driver, root) {
  ...
  const _locator = core.By.css(`.editButton`);
  const hasElement = await _element.containsElement(_locator);
  if (!hasElement) {
    return null;
  }
...
async function _utam_get_bodyIframe(driver, root) {
  ...
  const _locator = core.By.css(`force-aloha-page`);
  const hasElement = await _element.containsElement(_locator);
  if (!hasElement) {
    return null;
  }
...
class RecordCreationFormField extends core.UtamBasePageObject {
  ...
  async getBodyIframe() {
    ...
    element = new _AlohaPage__default["default"](driver, element);
...
module.exports = RecordCreationFormField;
