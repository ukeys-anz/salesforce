...
async function _utam_get_tabset2(driver, root) {
  let _element = root;
  const _locator = core.By.css(`flexipage-tabset2`);
  return _element.findElement(_locator);
}

class RecordHomeTemplateDesktop2 extends core.UtamBasePageObject {
  ...
  async getTabset2() {
    const driver = this.driver;
    const root = await this.getRootElement();
    let element = await _utam_get_tabset2(driver, root);
...
module.exports = RecordHomeTemplateDesktop2;
