...
async function _utam_get_cardHolder(driver, root) {
  ...
  const _locator = core.By.css("[data-test-id='cardholder']");
  return _element.findElement(_locator);
}

async function _utam_get_last4Digits(driver, root) {
  let _element = await _utam_get_lightningCard(driver, root);
  const _locator = core.By.css("[data-test-id='last-4-digits']");
  return _element.findElement(_locator);
}

async function _utam_get_expiryDate(driver, root) {
  let _element = await _utam_get_lightningCard(driver, root);
  const _locator = core.By.css("[data-test-id='expiry-date']");
  return _element.findElement(_locator);
}

async function _utam_get_status(driver, root) {
  let _element = await _utam_get_lightningCard(driver, root);
  const _locator = core.By.css("[data-test-id='status']");
  return _element.findElement(_locator);
}
 ...
 * @author UTAM
 */
class LwcViewCardsDetails extends core.UtamBasePageObject {
...
module.exports = LwcViewCardsDetails;
