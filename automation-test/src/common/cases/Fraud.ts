...
export default class Fraud extends Case implements IAssignNewOwner {
  ...
  async create(caseData: any): Promise<void> {
    ...
    await recordCreationFormRoot.selectCaseRecordType(CaseType.FRAUD);
    ...
    Fraud.creationFormFieldIndexMap = await creationFormUtils.fillInFields(
      this.sobject,
      Fraud.creationFormFieldIndexMap,
      fieldsToFill
    );
