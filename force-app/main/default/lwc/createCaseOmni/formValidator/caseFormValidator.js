 ...
 //  Validate Required fields for Customer Complaint
  async function validateCustomerComplaint(omniJsonData) {
    ...
    if (caseDetails.isThisCustomerComplaint === CUSTOMER_AGREES) {
      checkFields(
        caseDetails.CustomerDetails,
        omniJsonData.custMap
      );
      checkCustomerIdentifier(caseDetails.CustomerDetails);

      cmpMap.push({ AccountPolicyNumber: "Account/Policy Number" });
      checkCommonValidations(caseDetails.ComplaintDetails, caseDetails, omniJsonData);
    }
...
function checkCommonValidations(Complaintdetails, caseDetails, omniJsonData) {
    ...
    if (Complaintdetails.thirdPartyRepCheckbox === CUSTOMER_AGREES) {
      checkFields(Complaintdetails,omniJsonData.thirdPartyMap);
    }
    ...
    if (Complaintdetails.Issue3Checkbox === CUSTOMER_AGREES) {
      let thirdCmpMap = JSON.parse(
        JSON.stringify(omniJsonData.thirdCmpMap)
      );
      checkForAccountPolicyNumber(Complaintdetails, thirdCmpMap, "3",caseDetails);
...
function checkForAccountPolicyNumber(details, issueMap, issueNumber,caseDetails) {
    if (caseDetails.isThisCustomerComplaint === CUSTOMER_AGREES) {
      if (issueNumber === "2") {
        issueMap.push({
...
function validateTertiaryComplaintRemedies(details,omniJsonData) {
  ...
  function  checkReferredToFirm(complaintRemedy, detailsOfComplaint, manufacturerNumber) {
