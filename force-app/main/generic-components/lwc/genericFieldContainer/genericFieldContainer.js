...
export default class GenericFieldContainer extends LightningElement {
      ...
      if (this._value && this._objectConfig) {
        ...
        if (Array.isArray(this._value)) {
          ...
          this._objectConfig.schema.forEach((field, ind) => {
            ...
            //flatten money object
            if (
              dtVal &&
              typeof dtVal === "object" &&
              "currencyCode" in dtVal &&
              "units" in dtVal
            ) {
              const units = parseInt(dtVal.units, 10) || 0;
              const nanos = dtVal.nanos || 0;
              dtVal = units + nanos / 1000000000;
            }
  ...
  formatDateValue(value) {
    ...
    //Return null if none above are met
    return null;
  }
