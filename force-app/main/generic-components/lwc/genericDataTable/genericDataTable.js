...
export default class GenericDataTable extends NavigationMixin(
  ...
  /**
   * Public API property for table data with processing
   */
  @api
  get tableData() {
  ...
  /**
   * Public getter/setter for table configuration
   */
  @api
  get tableConfig() {
  ...
  set tableConfig(value) {
    try {
      ...
      if (parsedConfig.dataPath && !this.extractPath) {
        this.extractPath = parsedConfig.dataPath;
      }
      if (parsedConfig.filterField && !this.filterField) {
        this.filterField = parsedConfig.filterField;
      }
      if (parsedConfig.filterValue && !this.filterValue) {
        this.filterValue = parsedConfig.filterValue;
      }
  ...
  async initialiseConfig() {
    if (this.configName) {
      try {
        const config = await getConfig({ configName: this.configName });
        if (config) {
  ...
  handleMessage(message) {
    ...
    this._hasError = false;
    this._isLoading = false;
    this.tableData = message.message;
  }
  ...
  _extractData(parsedData) {
    try {
      ...
      if (this.extractPath) {
        ...
        const dataAtPath = pathExtracted[this.extractPath];

        if (!dataAtPath) {
          console.warn(
            `GenericDataTable: No data found at path: ${this.extractPath}`
          );
          return [];
        }
    ...
    try {
      ...
      const processedData = extractedData.map((item, index) => {
        ...
        const processedItem = {
          ...
          id: (this.keyField && item[this.keyField]) || `row_${index}`
        };
  ...
  _filterData(dataArray) {
    ...
    try {
      const matchingItem = dataArray.find((item) => {
        const fieldValues = extractValuesByPaths(item, this.filterField);
        const actualValue = fieldValues[this.filterField];
        return actualValue === this.filterValue;
      });
  ...
  handleRowAction(event) {
    ...
    const navigationConfig =
      actionConfig?.navigationConfig || this.tableConfig?.navigationConfig;
