  ...
  describe("Field Type Tests - Maximum Code Coverage", () => {
    ...
    it("renders lightning-formatted-number for number field type without attributes", async () => {
      ...
      element.field = {
        ...
        // No attributes - this is required for isNumber to return true
      };
    ...
    it("renders lightning-formatted-number with currency formatting for currency field type", async () => {
      const element = createElement("c-generic-field", {
        is: GenericField
      });
    ...
    it("does not render as number when field has attributes (should use currency or other formatting)", async () => {
      const element = createElement("c-generic-field", {
        is: GenericField
      });
      ...
      // Since this field has attributes, isNumber should return false
      // and it should fall back to the placeholder template
      const numberFields = element.shadowRoot.querySelectorAll(
        "lightning-formatted-number"
      );
      expect(numberFields.length).toBe(0);
    });

    it("handles currency field without format-style attribute", async () => {
      const element = createElement("c-generic-field", {
        is: GenericField
      });
      ...
      // Should not render as currency since format-style is not "currency"
      const currencyFields = element.shadowRoot.querySelectorAll(
        "lightning-formatted-number[format-style='currency']"
      );
      expect(currencyFields.length).toBe(0);
    });

    it("handles field with attributes but no format-style property", async () => {
      const element = createElement("c-generic-field", {
        is: GenericField
      });
      ...
      element.field = {
        ...
        attributes: {
          ...
          // No format-style property at all
        }
      };
      element.value = "Test value";
      element.paddingSize = { fieldLabel: "3", fieldValue: "9" };
      ...
      // isCurrency should return false when format-style property doesn't exist
      const currencyFields = element.shadowRoot.querySelectorAll(
        "lightning-formatted-number[format-style='currency']"
      );
      expect(currencyFields.length).toBe(0);
    ...
    it("handles field without attributes for isCurrency check", async () => {
      const element = createElement("c-generic-field", {
        is: GenericField
      });
      ...
      // isCurrency should return false when field.attributes doesn't exist
      const currencyFields = element.shadowRoot.querySelectorAll(
        "lightning-formatted-number[format-style='currency']"
      );
      expect(currencyFields.length).toBe(0);
