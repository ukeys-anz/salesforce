...
export default class cBaseCombobox extends LightningElement {
  ...
  processItem(item) {
    ...
    if (itemCopy.selectable) {
      ...
      if (!this.multiSelect && item.highlight) {
  ...
  filterItems(searchString) {
    if (searchString && searchString.length > 0) {
      this.message = "";
      if (searchString.length >= this.minChar) {
        let flag = true;
...
//            this.filterOptionsData.push(this._items[i]);
            ...
            this._items[i].isVisible = false;
          }
        }
        if (flag) {
          this.message = "No results found for '" + searchString + "'";
        }
      }
  ...
  dropdownKeyboardInterface() {
    ...
    return {
      ...
      selectByIndex(index) {
        ...
        if (that.multiSelect) {
          that._highlightedItemIndex = index;
        }
      },
