...
export default class LoanPreferenceDetailsDisplay extends LightningElement {
  ...
  loadLoanPreferenceData() {
    this.componentSpinner = true;
    Promise.all([
      getLoanPreference({ loanId: this.recordId }),
      getAdditionFundsForLoanPreference({ recordId: this.recordId }),
      getCalculateLoanRepayment({ loanId: this.recordId })
    ])
      .then(
        ([
          loanPreferenceViewModel,
          additionalFunds,
  ...
  // Convert loan repayment type, intrest rate type and repayment frequency into readable labels
