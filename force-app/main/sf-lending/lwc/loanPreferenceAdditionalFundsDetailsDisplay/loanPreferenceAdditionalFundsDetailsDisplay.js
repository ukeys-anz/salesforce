...
export default class LoanPreferenceAdditionalFundsDetailsDisplay extends LightningElement {
  ...
  getPurposeMappingAndQueryData() {
    ...
    const purCatMapping = getCategoryToPurposeMapping();
    Promise.all([preferenceResult, purCatMapping])
      .then(([addFundsData, purCatMappingData]) => {
        this.additionFunds = addFundsData;
        this.purposeValueMapping = purCatMappingData;
        if ((this.additionFunds?.additionalFunds?.length ?? 0) === 0) {
          return;
        }
        this.checkIfAddFundExist = true;
        this.categoryWithAssociatedPurposeMap =
          this.transformDataWithPurposeCategoryMapping();
      })
      .catch((error) => {
        this.error = error;
        this.additionFunds = [];
        this.purposeValueMapping = [];
      })
      .finally(() => {
  ...
  //Below method will transform the data and include the Purpose and Category mapping
  transformDataWithPurposeCategoryMapping() {
    this.additionFundDetails = [];
    let fundsData = this.additionFunds.additionalFunds;
    fundsData.forEach((purposeValue) => {
      const categoryAndLabelDetails = this.fetchCategoryAndLabelForPurposeValue(
        purposeValue.purpose
      );
      if (categoryAndLabelDetails === null) {
        return;
      }
      let transformAddFunds = {
        Purpose: purposeValue.purpose,
        PurposeCategory: categoryAndLabelDetails.Category,
        imageSrc: this.imageForCategory.get(categoryAndLabelDetails.Category),
        helpText: this.helpTextForCategory.get(
          categoryAndLabelDetails.Category
        ),
        PurposeLabel: categoryAndLabelDetails.Label,
        Amount: purposeValue.amount,
        OtherReason: purposeValue.otherReason
      };
      this.additionFundDetails.push(transformAddFunds);
    });
    return this.categoryWithAssociatedPurposeMapping(this.additionFundDetails);
  }
  ...
  categoryWithAssociatedPurposeMapping(additionFundRecords) {
    ...
    additionFundRecords.forEach((fundRecord) => {
      const key = fundRecord.PurposeCategory || "Purpose Value";
    ...
    return [...this.purposeSubPurposeMap].map(([name, value]) => ({
      name,
      value,
      imgSrc: value[0]?.imageSrc,
      helpText: value[0]?.helpText
    }));
  }

  //Method to get the Category and Label based on the Purpose Value
  fetchCategoryAndLabelForPurposeValue(purposeValue) {
    if (purposeValue === null) {
      return null;
    }
    const category = this.purposeValueMapping.find((cat) =>
      cat.purpose.some((purpose) => purpose.value === purposeValue)
    );

    if (category) {
      const purposeLabel = category.purpose.find(
        (purpose) => purpose.value === purposeValue
      );
