...
import updateSOPAndParty from "@salesforce/apex/BOHController.updateSOPAndParty";
...
export default class AboutYouViewDetails extends LightningElement {
  ...
  async updateSOPAndParty() {
    this.componentSpinner = true;
    try {
      let response = await updateSOPAndParty({
        sopModel: this.filteredSOPData,
        loanId: this.recordId
      });
      //Updating data if callout success
      if (
        response != null &&
      ...
      else if (!response?.isUpdateSOPFailed || !response?.isUpdatePartyFailed) {
        this.refreshData();
